...
export default function SellerEditProduct({ params }: { params: { id: string } }) {
  const existing = useMemo(() => products.find((p) => p.id === params.id), [params.id]);
  ...
  if (!existing) {
    return <main className="max-w-3xl mx-auto p-6">Product not found.</main>;
  }
